...
from .panel import OBJECT_PT_MainPanel, OBJECT_PT_IPAdapterPanel, OBJECT_OT_OpenNewIPAdapterImage, OBJECT_PT_LoRAPanel
...
class UninstallAddonOperator(bpy.types.Operator):
    """Operator to uninstall the addon"""
    bl_idname = "diffused_texture_addon.uninstall_addon"
    bl_label = "Uninstall Addon"
    bl_description = "Uninstall the DiffusedTexture addon."
    bl_options = {"REGISTER", "INTERNAL"}

    def execute(self, context):
        module_name = "diffused_texture_addon"  # Replace with your actual module name
        try:
            bpy.ops.preferences.addon_remove(module=module_name)
            self.report({"INFO"}, f"Addon '{module_name}' uninstalled successfully.")
        except Exception as e:
            self.report({"ERROR"}, f"Failed to uninstall addon: {str(e)}")
            return {"CANCELLED"}
...
class DiffuseTexPreferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        layout.operator(InstallModelsOperator.bl_idname, text="Install Models", icon="IMPORT")
        layout.operator(UninstallAddonOperator.bl_idname, text="Uninstall Addon", icon="CANCEL")
